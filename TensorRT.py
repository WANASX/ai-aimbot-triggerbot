...
# Reduce logging in inner loops; only log INFO messages occasionally
logging.basicConfig(level=logging.INFO, format='[%(asctime)s] %(levelname)s:%(name)s:%(message)s')

# TensorRT logger and runtime initialization
TRT_LOGGER = trt.Logger(trt.Logger.WARNING)
runtime = trt.Runtime(TRT_LOGGER)

# Global configuration flags and parameters
aimbot_enabled = True
...
# Pre-set tensor addresses (constant over runtime)
...
def capture_screen():
    ...
    try:
        ...
        while not exit_event.is_set():
            ...
            frame_bgr = cv2.cvtColor(np.array(frame_raw), cv2.COLOR_BGRA2BGR)
            with frame_lock:
...
def move_mouse_towards_target(delta_x, delta_y, max_speed=350):
    ...
    # Note: Debug logging is removed from the inner loop for performance
...
def trigger_action():
    ...
    try:
        while trigger_enabled and trigger_active and detection_valid and not exit_event.is_set():
...
# Inference loop counter to limit logging frequency
frame_count = 0

# Main inference loop optimized for TensorRT performance and accuracy
# Preprocessing uses cv2.dnn.blobFromImage to combine resizing and normalization in C++.
while not exit_event.is_set():
    ...
    # Preprocess the frame using blobFromImage (scales pixel values by 1/255, no mean subtraction, no swapRB)
